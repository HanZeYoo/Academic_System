...
class TeacherAcademicEvaluationScreen extends StatefulWidget {
  ...
  const TeacherAcademicEvaluationScreen({super.key, required this.username});

  @override
  State<TeacherAcademicEvaluationScreen> createState() =>
      _TeacherAcademicEvaluationScreenState();
}

class _TeacherAcademicEvaluationScreenState
    extends State<TeacherAcademicEvaluationScreen> {
  // ---------- state ----------
  ...
  Future<void> _loadClasses() async {
    ...
    if (teacher == null) { setState(() => _isLoading = false); return; }
  ...
  /// Falls back to simple average if no assessment setup weights.
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Students Needing Attention Card — live count
  ...
  Widget _buildStatCard(
    ...
    // Calculate width based on screen size to make Wrap responsive
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                return Wrap(
                  ...
                  runSpacing: 16,
                  children: [
                    _buildComponentAvg('Quizzes', quizAvg > 0 ? '${quizAvg.toStringAsFixed(1)}%' : '--', Icons.help_outline,
                        const Color(0xFF3B82F6)),
                    _buildComponentAvg('Assignments', asgAvg > 0 ? '${asgAvg.toStringAsFixed(1)}%' : '--',
                        Icons.description_outlined, const Color(0xFF10B981)),
                    _buildComponentAvg('Activities', actAvg > 0 ? '${actAvg.toStringAsFixed(1)}%' : '--', Icons.star_border,
                        const Color(0xFFF59E0B)),
                    _buildComponentAvg('Project', prjAvg > 0 ? '${prjAvg.toStringAsFixed(1)}%' : '--',
          ...
          Wrap(
            ...
            children: [
              ...
              _buildOutlinedButton(
                  Icons.chat_bubble_outline, 'Remarks', const Color(0xFF3B82F6), onTap: onRemarks),
            ],
          ),
        ],
      ),
...
// Simple mock chart painter to mimic the curve in the design
