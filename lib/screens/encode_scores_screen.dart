...
class _EncodeScoresScreenState extends State<EncodeScoresScreen> {
  ...
  Future<void> _saveScores() async {
    ...
    if (hasEmptyScore) {
      if (mounted) {
        _showErrorDialog('Missing Scores', 'All student scores are required. Please enter a score for each student.');
      }
      return;
    }
    ...
    if (hasInvalidScore) {
      if (mounted) {
        _showErrorDialog('Invalid Score', 'Invalid score entered. Scores must be numbers between 0 and $_totalScore.');
      }
      return;
    }
